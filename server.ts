...
app.get("/api/android/download", async (req, res) => {
  try {
    ...
    // Default: Serve clean Android WebAPK Installer app page that triggers 1-tap installation
...
<head>
  ...
  <style>
    ...
    h1 { margin-top: 12px; font-size: 22px; color: #FF8A50; }
    ...
    .logo { width: 80px; height: 80px; border-radius: 20px; border: 2px solid #FF6A3D; }
  </style>
</head>
<body>
  <div class="card">
    <img src="${appHost}/sofi-logo.jpg" class="logo" alt="Sofi Logo" />
    <h1>Sofi AI Assistant - Android App</h1>
    <p>Official WebAPK standalone installation for Android 9, 10, 11, 12, 13, 14 & 15. Bypasses package parse errors.</p>

    <button id="installBtn" class="btn">📲 Tap to Install App on Android</button>
    <a href="${appHost}" class="btn btn-secondary">🌐 Open Live App in Browser</a>
    <a href="${appHost}/api/android/download?file=raw" class="btn btn-secondary" style="opacity: 0.6; font-size:11px;">📦 Download Developer Zip Archive</a>
  </div>
  ...
  <script>
    ...
    window.addEventListener('beforeinstallprompt', (e) => {
      ...
      if (btn) btn.innerText = "📲 Tap to Install Sofi App Now";
    });
    ...
    document.getElementById('installBtn').addEventListener('click', async () => {
      if (deferredPrompt) {
        ...
        if (choice.outcome === 'accepted') {
          alert('Sofi AI Assistant has been installed on your Android device!');
        }
      } else {
